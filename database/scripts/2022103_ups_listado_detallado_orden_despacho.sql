DROP PROCEDURE `ups_listado_detallado_orden_despacho`;
CREATE PROCEDURE `ups_listado_detallado_orden_despacho`(IN `p_id_tercero` INT, IN `p_id_pedido` INT, IN `p_fecha_inicio` DATE, IN `p_fecha_fin` DATE) NOT DETERMINISTIC NO SQL SQL SECURITY DEFINER SELECT
iod.id,
iod.id_pedido,
iod.fecha_despacho,
